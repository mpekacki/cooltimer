...
function verifyTotalWorkedTime(container, hours, minutes, seconds) {
  return expect(container.getByTestId("totalWorkedTime").textContent).toBe(formatSecondsAsText(hours * 60 * 60 + minutes * 60 + seconds));
}

function verifyTotalCombinedTime(container, hours, minutes, seconds) {
  return expect(container.getByTestId("totalCombinedTime").textContent).toBe(formatSecondsAsText(hours * 60 * 60 + minutes * 60 + seconds));
}

function verifyAvailableBreakTime(container, hours, minutes, seconds) {
  return expect(container.getByTestId("availableBreakTime").textContent).toBe(formatSecondsAsText(hours * 60 * 60 + minutes * 60 + seconds));
}
